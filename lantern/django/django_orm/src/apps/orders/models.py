...
class Order(BaseDateAuditModel):
    ...
    car_id = models.ForeignKey(to='Car', db_column='car', blank=True, null=False, on_delete=models.DO_NOTHING,
                               db_constraint=False)
    status = models.TextField(max_length=50, choices=STATUS_CHOICES, default=STATUS_EXPECTED, blank=True)
    ...
    email = models.EmailField(max_length=32, blank=True, null=False, unique=True)
    phone = models.IntegerField(max_length=20, blank=True, null=False, unique=True)
    message = models.CharField(max_length=254, blank=True, null=False)
